...
Поэтому для текущей версии допустим только локальный Ollama. Облачный режим появится позже вместе с тонким серверным BFF/proxy, который хранит ключ и применяет ограничения запросов. До этого приложение не должно просить пользователя вводить облачный API-ключ даже для временного хранения в памяти вкладки.
